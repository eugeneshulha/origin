describe CorevistAPI::Filters::Links::MaintainedByUserLink, type: :filter do
  let(:instance) { build(:links_maintained_by_user_link) }
  ...
  describe '#perform' do
    ...
    context 'when params exclude maintained by user' do
      let(:data) { build(:results_user_result, :without_maintained_by_user) }
    ...
    context 'when params include maintained by user' do
      let(:data) { build(:results_user_result, :with_maintained_by_user) }
