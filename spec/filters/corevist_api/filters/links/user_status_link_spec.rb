describe CorevistAPI::Filters::Links::UserStatusLink, type: :filter do
  let(:instance) { build(:links_user_status_link) }
  ...
  describe '#perform' do
    ...
    context 'when params exclude user status' do
      let(:data) { build(:results_user_result, :without_user_status) }
    ...
    context 'when params include include user status' do
      context 'when user status is valid' do
        let(:data) { build(:results_user_result, :with_valid_user_status) }
      ...
      context 'when user status is invalid' do
        let(:data) { build(:results_user_result, :with_invalid_user_status) }
