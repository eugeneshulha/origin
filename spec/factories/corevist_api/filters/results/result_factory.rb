FactoryBot.define do
  factory :results_base_result, class: 'CorevistAPI::Filters::Results::BaseResult' do
    object { create(:user) }
    params { {} }
  ...
  factory :results_user_result, class: 'CorevistAPI::Filters::Results::UserResult' do
    object { create(:user) }
    params { {} }
    query { CorevistAPI::User }
    skip_create
    ...
    trait :with_partner_number_param do
      params { { sold_to_number: create(:partner) } }
    end
    ...
    trait :with_partner_number do
      partner_number { create(:partner).number }
    end

    trait :with_sold_tos_and_ship_tos do
      object { create(:user, :with_sold_tos_and_ship_tos) }
    end

    trait :with_sold_tos do
      object { create(:user, :with_sold_tos) }
    end

    trait :with_ship_tos do
      object { create(:user, :with_ship_tos) }
    end
