  ...
  module RFCServices::BaseRFC::Conversion
    ...
    def user_to_rfc(user)
      rfc_user =  {
        ...
        ASSIGNED_SOLD_TOS => user.assigned_sold_tos.map { |sold_to| { NR => sold_to.number.add_leading_zeros, SA => sold_to.sales_area.title } },
        ASSIGNED_SHIP_TOS => user.assigned_ship_tos.map { |ship_to| { NR => ship_to.number.add_leading_zeros, SA => ship_to.sales_area.title } },
        ASSIGNED_PAYERS => user.assigned_payers.map { |payer| { NR => payer.number.add_leading_zeros, SA => payer.sales_area.title } }
